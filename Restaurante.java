...
public class Restaurante {
     ...
     * @param numeroMesas que puede albergar en su espacio.
    ...
    public Restaurante(int numeroMesas) {
        ...
        int[] capacidades = {2,4,6};// lista con las capacidades de las mesas.
        // Inicializar mesas con diferentes capacidades
        for (int i = 1; i <= numeroMesas; i++) {
            mesas.add(new Mesa(i, capacidades[i % capacidades.length]));  // Alterna entre mesas de 2, 4, y 6 personas
        }
    }

    /**Metodo
     * Hace el registro de una persona que ha llamado para una reservacion.
     * Indica la fecha de dicha reservacion y se le cobra de acuerdo con el tiempo que falte para esa fecha.
    ...
    public void registrarReserva(Cliente cliente, LocalDateTime fechaHora, int numeroPersonas, Precio precio) {
        ...
        precio.cobrar(fechaHora);
        //Condicional de que si hay una mesa.
        if (mesaDisponible != null) {
            if ( precio.getTotal() != -1){
    ...
    /**Metodo
     * Muestra en pantalla todas las reservaciones de una fecha en específico.
     * @param fecha
     */
